...
func GetLocalIP() string {
	...
	if libprocessIP := os.Getenv("LIBPROCESS_IP"); libprocessIP != "" {
		...
		if address != nil {
			...
			wlogs.Info("LibProess IP = %s", libprocessIP)
			return libprocessIP
		}
	...
	for _, address := range addrs {
		...
		if ipnet, ok := address.(*net.IPNet); ok && !ipnet.IP.IsLoopback() {
			if ipnet.IP.To4() != nil {
				wlogs.Info("InterfaceAddress = %s", ipnet.IP.String())
...
func (exec *WorkloadExecutor) Registered(driver exec.ExecutorDriver, execInfo *mesos.ExecutorInfo, fwinfo *mesos.FrameworkInfo, slaveInfo *mesos.SlaveInfo) {
	wlogs.Info("Registered Executor on slave ") //, slaveInfo.GetHostname())
}
...
func (exec *WorkloadExecutor) LaunchTask(driver exec.ExecutorDriver, taskInfo *mesos.TaskInfo) {
	wlogs.Info("Launching task", taskInfo.GetName(), "with command", taskInfo.Command.GetValue())
	...
	wlogs.Info("The Taskmon object = %v\n", *M)
	...
	go func() {
		...
		if err != nil {
			wlogs.Info("Got error", err)
		}

		wlogs.Info("Total tasks launched ", exec.tasksLaunched)
		...
		// finish task
		wlogs.Info("Finishing task", taskInfo.GetName())
		...
		wlogs.Info("Task finished", taskInfo.GetName())
	}()
}
...
func (exec *WorkloadExecutor) KillTask(driver exec.ExecutorDriver, taskID *mesos.TaskID) {
	...
	wlogs.Info("Killed task with task id:", tid)
}
...
func main() {
	wlogs.Info("Starting Workload Executor")
	...
	if err != nil {
		wlogs.Info("Unable to create a ExecutorDriver ", err.Error())
	}
	...
	wlogs.Info("Executor process has started and running.")
	_, err = driver.Join()
	if err != nil {
		wlogs.Info("driver failed:", err)
	}
	wlogs.Info("Executor Finished, Delete all the containers")
	for _, M := range WorklaodExec.monMap {
		M.Die()
	}
	wlogs.Info("executor terminated")
}
